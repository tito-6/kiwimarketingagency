...
export function BlogTicker() {
  ...
  return (
    <div className="overflow-hidden border-y border-neutral-900/10 bg-neutral-50/80 py-4 backdrop-blur-sm">
      <div className="flex w-max gap-10" style={{ animation: "marquee 70s linear infinite" }}>
        {row.map((item, i) => (
          <span
            key={`${item}-${i}`}
            className="flex items-center gap-10 text-sm font-medium uppercase tracking-[0.25em] text-neutral-900/40"
          >
            {item}
