...
export function IletisimChannels() {
  return (
    <section className="border-y border-neutral-900/10 py-20">
        ...
        {iletisimPage.channels.map((channel, i) => (
          <ClipReveal key={channel.id} delay={i * 0.1} direction={i === 0 ? "up" : i === 2 ? "right" : "left"}>
            <Link
              ...
              className="group relative block overflow-hidden rounded-3xl border border-neutral-900/10 bg-neutral-50 p-8 transition-colors hover:border-kiwi-400/40"
            >
              <motion.div
                className="pointer-events-none absolute inset-0 bg-kiwi-400/0 transition-colors group-hover:bg-kiwi-400/5"
              />
              <motion.span
                className="flex h-14 w-14 items-center justify-center rounded-2xl border border-neutral-900/10 bg-neutral-900/[0.03] text-2xl"
              ...
              <p className="mt-2 text-xl font-medium text-neutral-900 group-hover:text-kiwi-400">
                {channel.value}
              </p>
              <p className="mt-3 text-sm text-neutral-900/40">{channel.description}</p>
              <motion.span
                className="mt-6 inline-flex items-center gap-2 text-sm text-neutral-900/50"
                whileHover={{ x: 6 }}
              >
                Bağlan <span>→</span>
              </motion.span>
